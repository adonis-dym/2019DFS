...
def main():
    ...
    # checking point
    parser.add_argument('--resume', type=str, default='/data/PreTrainedModel/Seg_checkpoint.pth.tar',
    ...
    model = model.cuda()
    output_dir = './Result/Split-Val/'
    crfoutput_dir = './Result/Split-Valcrf/'
    ...
    for i, sample in enumerate(tbar):
        ...
        if args.crf==False:
            ...
            for j in range(len(name)):     
                ...
                #image = CNNMap
                AGLName = os.path.join(crfoutput_dir, name[j].replace(IMG_FILE_STR, DEPTH_FILE_STR))                
                im = np.expand_dims(tifffile.imread(AGLName),-1)
                
                softmax = pred[j].reshape((5,-1))     
                ...
                d = dcrf.DenseCRF(im.shape[0] * im.shape[1], 5)
                ...
                feats = create_pairwise_bilateral(sdims=(10, 10), schan=[13],
                                                img=im, chdim=2)
                ...
                crfoutName = name[j].replace(IMG_FILE_STR, CLSPRED_FILE_STR)  
                crfpred = res.astype('uint8')           
                crfpred = convert_labels(crfpred, toLasStandard=True)
